...
// Server-side Stripe instance (for server actions)
...
export const getStripeServer = async () => {
  if (!stripeServerInstance) {
    ...
    if (!secretKey || secretKey === 'sk_test_placeholder') {
      console.warn('⚠️ Stripe secret key not found. Payment processing will fail.')
      return null
    }
...
// Real Stripe functions (updated implementations)
...
export async function confirmPaymentIntent(paymentIntentId: string, paymentMethodId?: string): Promise<StripePaymentIntent> {
  try {
    const stripe = await getStripeServer()
    
    if (!stripe) {
      // Fallback to test mode
      console.log('🔮 Test mode - confirmPaymentIntent called:', { paymentIntentId, paymentMethodId })
      return {
        id: paymentIntentId,
        amount: 0,
        currency: 'eur',
...
export async function retrievePaymentIntent(paymentIntentId: string): Promise<StripePaymentIntent> {
  try {
    ...
    if (!stripe) {
      // Fallback to test mode
      console.log('🔮 Test mode - retrievePaymentIntent called:', paymentIntentId)
      return {
        id: paymentIntentId,
        amount: 0,
        currency: 'eur',
    ...
    // Real Stripe payment intent retrieval
...
export function getStripePublishableKey(): string {
  ...
  if (!key) {
    console.warn('⚠️ NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY not found in environment variables')
