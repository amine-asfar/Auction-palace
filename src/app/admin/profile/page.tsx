...
export default function AdminProfilePage() {
  ...
  // Vérification du rôle admin (dans un vrai projet, cela viendrait de la base de données)
  const isAdmin = true // Pour la démo, supposons que l'utilisateur est admin

  // Données statiques pour l'admin
  ...
  // Données statiques des utilisateurs à vérifier
  ...
  useEffect(() => {
    ...
    if (!isLoading && isAuthenticated && !isAdmin) {
      router.push("/profile") // Rediriger vers le profil normal si pas admin
    }
  ...
  const handleVerifyUser = (userId: string) => {
    setUsersToVerify(users => 
      users.map(user => 
        user.id === userId 
          ? { ...user, identity_status: 'verified' as const, updated_at: new Date().toISOString() }
          : user
      )
    )
    toast({
  ...
  const handleRejectUser = (userId: string) => {
    setUsersToVerify(users => 
      users.map(user => 
        user.id === userId 
          ? { ...user, identity_status: 'rejected' as const, updated_at: new Date().toISOString() }
          : user
      )
    )
    toast({
  ...
  const handleDeleteDocument = (userId: string) => {
    setUsersToVerify(users => 
      users.map(user => 
        user.id === userId 
          ? { ...user, identity_document_url: undefined, identity_status: 'pending' as const, updated_at: new Date().toISOString() }
          : user
      )
    )
    toast({
  ...
  const handleViewDocument = (documentUrl?: string) => {
    if (documentUrl) {
      ...
      toast({
        ...
        description: "Aucun document d'identité n'est disponible pour cet utilisateur.",
        variant: "destructive"
      })
    }
  }
  ...
  const pendingCount = usersToVerify.filter(user => user.identity_status === 'pending').length
  const verifiedCount = usersToVerify.filter(user => user.identity_status === 'verified').length
  const rejectedCount = usersToVerify.filter(user => user.identity_status === 'rejected').length
        ...
        <Card className="border-indigo-100 shadow-sm">
          <div className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-xl font-semibold">{adminProfile.first_name} {adminProfile.last_name}</h2>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-indigo-50">
                    <TableHead className="text-indigo-700">Utilisateur</TableHead>
                    <TableHead className="text-indigo-700">Email</TableHead>
                    <TableHead className="text-indigo-700">Statut</TableHead>
                    <TableHead className="text-indigo-700">Date de création</TableHead>
                    <TableHead className="text-indigo-700">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {usersToVerify.map((user) => {
                    const statusConfig = getIdentityStatusConfig(user.identity_status)
                    ...
                    return (
                      <TableRow key={user.id} className="hover:bg-indigo-50/50">
                        ...
                        <TableCell>
                          <Badge variant="outline" className={`${statusConfig.bgColor} ${statusConfig.borderColor} ${statusConfig.color}`}>
