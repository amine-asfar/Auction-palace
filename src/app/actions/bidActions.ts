...
export async function placeBid(
  ...
  if (bid_amount > 99999999.99) {
    throw new Error(
      "Le montant de l'enchère ne peut pas dépasser 99,999,999.99€",
    );
  }
  ...
  if (currentPrice === startingPrice && bid_amount < startingPrice) {
    throw new Error(
      `La première enchère doit être au moins égale au prix de départ (${startingPrice}€)`,
    );
  }

  const { data: bid, error: bidError } = await (
    await supabase
  )
    .from("Bids")
    .insert([
...
export async function getBids(productId: string) {
  ...
  const bidsWithProfiles = bids.map((bid) => {
    const userProfile = profiles?.find(
      (profile) => profile.user_id === bid.user_id,
    );
    return {
      ...bid,
  ...
  const purchaseHistory =
    data?.filter((bid) => {
      ...
      const isAuctionCompleted =
        product.status === "completed" || isAuctionEnded;
